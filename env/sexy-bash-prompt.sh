...
if [ $(id -u) -eq 0 ]; then
  ...
  sexy_bash_prompt_user_color="$(tput bold)$(tput setaf 9)";
else
  # PROMPT_USER_COLOR="$(tput bold)$(tput setaf 9)"
  sexy_bash_prompt_symbol="\$"
  sexy_bash_prompt_user_color="$sexy_bash_prompt_bold$(tput setaf 4)"
fi

# Apply any color overrides that have been set in the environment
# if [ $(id -u) -eq 0 ]; then
#   sexy_bash_prompt_user_color="$(tput bold)$(tput setaf 9)";
# elif [[ -n "$PROMPT_USER_COLOR" ]]; then 
#   sexy_bash_prompt_user_color="$PROMPT_USER_COLOR";
# fi
...
export PS1="\[$sexy_bash_prompt_reset\]\
