...
function mysql_install() 
{
    ...
    case `plateform` in 
        redhat)
			local MYSQL_RPM="mysql-community-server-5.7.36-1.el7.x86_64.rpm"
			if [[ $_version =~ ^8.*$ ]];
			then 
				MYSQL_RPM=mysql80-community-release-el8-1.noarch.rpm
				MYSQL_RPM=mysql-community-server-8.0.27-1.el8.x86_64.rpm
			fi
