...
fuseki_install() {
    ...
    echo "https://$repo_url/jena/binaries/jena-fuseki-war-$version.war"
    curl -fSL https://$repo_url/jena/binaries/jena-fuseki-war-$version.war -o ${catalina_home}/webapps/$name.war
    ...
    # sudo tar -xzf /tmp/apache-jena-fuseki-$version.tar.gz
    # sudo mv /tmp/apache-jena-fuseki-${version} $INSTALL_DIR/$appName-$version
    ...
    #     # then 
            # sudo cp -f "/tmp/apache-jena-fuseki-${version}/fuseki.war" ${catalina_home}/webapps/$name.war
    #     # fi
